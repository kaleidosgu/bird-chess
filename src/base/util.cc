...
namespace base
{
    void util::TrimLeft(string & str, char c)
    {
        int nFirst = str.find_first_not_of(c);
        if (nFirst < 0)
        {
            str = "";
        }
        if (nFirst > 0 && (unsigned) nFirst <= str.size())
        {
            str = str.substr(nFirst, str.size() - nFirst);
        }
    }

    void util::TrimRight(string & str, char c)
    {
        int nLast = str.find_last_not_of(c);
        if (nLast < 0)
        {
            str = "";
        }
        if (nLast > 0 && (unsigned) nLast < str.size())
        {
            str = str.substr(0, nLast + 1);
        }
    }

    void util::TrimCR(string & str)
    {
    ...
    void util::Split(char delimiter, const char *s, vector < string > &out, bool ignoreEmpty /* = false */ )
    {
        //ASSERT(s);
    ...
    void util::ReplaceAll(string & s, const char *from, const char *to)
    {
        ...
        for (string::size_type pos = s.find(from);
                pos != string::npos; pos = s.find(from, pos + lenTo))
        {
            s.replace(pos, lenFrom, to);
        }
    }

    void util::ToLower(char *s)
    {
    ...
    bool util::mkdir(const std::string & path, int mode /* = 0777 */ )
    {
        ...
        if (pos != string::npos)
        {
                    ...
                    pos = path.find('/', pos + 1))
            {
                ...
                {
                    WriteLog(LEVEL_ERROR, "Unable to mkdir %s.\n", subpath.c_str());
                    return false;
                }
            }
        }
    ...
    static bool same(const char *file1, const char *file2)
    {
        ...
        if (f1 == NULL)
        {
            WriteLog(LEVEL_ERROR, "Unable to read %s.\n", file1);
            return false;
        }
...
exit_false:
    ...
    */
        ...
        /*
    {
        ...
        if (ddest == NULL)
        {
            WriteLog(LEVEL_ERROR, "Syncdir: unable to open dest %s.\n", dest.c_str());
            return;
        }
        ...
        {
            closedir(ddest);
            WriteLog(LEVEL_ERROR, "Syncdir: unable to open src %s.\n", src.c_str());
            return;
        }
        ...
        if (!todelete.empty())
        {
            for (set < string >::const_iterator iter = todelete.begin(); iter != todelete.end(); ++iter)
            {
                WriteLog("Sync: deleting %s.\n", iter->c_str());
        ...
        while ((e = readdir(dsrc)))
        {
            ...
            {
                WriteLog("Sync: updating %s.\n", fdest.c_str());
                if (keepSrc)
                {
                    ssystem((string("cp -R ") + src +
                                e->d_name + " " + dest).c_str());
                }
                else
                {
                    rename((src + e->d_name).c_str(),
                            (dest + e->d_name).c_str());
                }
            }
        }
    ...
    */
    ...
    int util::copy(const char *srcfile, const char *dstfile)
    {
        ...
        while (1)
        {
            ...
            {
                err = true;
                WriteLog(LEVEL_ERROR, "Read failed: %s.\n", safe_strerror(errno).c_str());
            }
            else if ((wbytes = write(dstFd, buf, rbytes)) != rbytes)
            {
                err = true;
                WriteLog(LEVEL_ERROR, "Write failed: %d, %s.\n", wbytes, safe_strerror(errno).c_str());
            }
            if (err)
            {
                close(srcFd);
    ...
    int util::directCopy(const char *srcfile, const char *dstfile)
    {
        ...
        while (1)
        {
            ...
            {
                err = true;
                WriteLog(LEVEL_ERROR, "Read sync failed: %s.\n", safe_strerror(errno).c_str());
            }
            else if (drop_cache(srcFd) == -1)
            {
                err = true;
                WriteLog(LEVEL_ERROR, "Read cache drop failed: %s.\n", safe_strerror(errno).c_str());
            }
            else if ((wbytes = write(dstFd, buf, rbytes)) != rbytes)
            {
                err = true;
            ...
            {
                err = true;
                WriteLog(LEVEL_ERROR, "write sync failed: %s.\n", safe_strerror(errno).c_str());
            }
            else if (drop_cache(dstFd) == -1)
            {
                err = true;
                WriteLog(LEVEL_ERROR, "write cache drop failed: %s.\n", safe_strerror(errno).c_str());
            }
            if (err)
            {
                close(srcFd);
    ...
    int util::ssystem(const char *command)
    {
        ...
        if (ret == -1)
        {
            WriteLog(LEVEL_ERROR, "System(\"%s\"): %s.\n", command, safe_strerror(errno).c_str());
        }
        else if (ret != 0)
        {
            WriteLog(LEVEL_ERROR, "Command failed: \"%s\".\n", command);
        }
        return ret;
    }
        ...
        while (*addpath)
        {
                    ...
                    && addpath[1] == '.')
            {
                ...
                if (pathlen == 1 && path[0] == '/')
                {
                        ...
                        || (pathlen == 3
                            && !memcmp(path + pathlen - 3,
                                "../", 3))
                        || (pathlen > 3
                            && !memcmp(path + pathlen - 4,
                                "/../", 4)))
                {
    ...
    {
        char *s = (char *)realloc(const_cast < void *>(buf1),
                size1 + size2 + 1);
    ...
    void util::string_vsnprintf(std::string & msg, const char *fmt,
            va_list ap)
    {
        int i = 0;
