...
namespace base
{
	...
	bool CryptDes::Des_Go(char *Out, char *In, int datalen, const char *Key, int keylen, bool Type)
	{
	...
	void CryptDes::SetKey(const char *Key, int len)
	{
		memset(deskey, 0, 16);
	...
	void CryptDes::DES(char Out[8], char In[8], const PSubKey pSubKey, bool Type)
	{
	...
	void CryptDes::SetSubKey(PSubKey pSubKey, const char Key[8])
	{
	...
	void CryptDes::F_func(bool In[32], const bool Ki[48])
	{
		static bool MR[48];
	...
	void CryptDes::S_func(bool Out[32], const bool In[48])
	{
	...
	void CryptDes::Transform(bool *Out, bool *In, const char *Table, int len)
	{
		for (int i=0; i<len; ++i)
		{
			Tmp[i] = In[Table[i] - 1];
	...
	void CryptDes::Xor(bool *InA, const bool *InB, int len)
	{
		for(int i=0; i<len; ++i)
		{
			InA[i] ^= InB[i];
	...
	void CryptDes::RotateL(bool *In, int len, int loop)
	{
		memcpy(Tmp, In, loop);
	...
	void CryptDes::ByteToBit(bool *Out, const char *In, int bits)
	{
	...
	void CryptDes::BitToByte(char *Out, const bool *In, int bits)
	{
		memset(Out, 0, bits>>3);
		for (int i=0; i<bits; ++i)
		{
	...
	unsigned int CryptDes::EncryptProcDES(char *Out, int dataLen, const char *Key, int keyLen)
	{
		char * In = Out;
	...
	unsigned int CryptDes::DecryptProcDES(char *Out, unsigned int dataLen, const char *Key, int keyLen)
	{ 
		char * In = Out;
		dataLen = (dataLen + 7) & 0xFFFFFFF8;
		SetKey(Key, keyLen);
